...
(()=>{
 ...
 function tickStudio(){if(studio&&hoverIndex===null&&innerWidth>760){const r=studio.getBoundingClientRect(),max=Math.max(1,studio.offsetHeight-innerHeight),p=clamp(-r.top/max,0,1),centers=[0,.5,1];words.forEach((w,j)=>{const focus=clamp(1-Math.abs(p-centers[j])*2.25,0,1);w.style.opacity=(.3+.7*focus).toFixed(3);w.style.transform=`translateX(${(11*focus).toFixed(1)}px)`});const idx=p<.25?0:(p<.75?1:2);renderStudio(idx)}requestAnimationFrame(tickStudio)}if(words.length){renderStudio(0,true);requestAnimationFrame(tickStudio)}
 // Expertise strips: hover/focus/tap reveal only; never auto-open on scroll.
 const serviceRows=[...document.querySelectorAll('.service-strip')];
 serviceRows.forEach((row,i)=>{const set=on=>serviceRows.forEach((r,j)=>r.classList.toggle('active',on&&j===i));row.addEventListener('mouseenter',()=>set(true));row.addEventListener('mouseleave',()=>set(false));row.addEventListener('focus',()=>set(true));row.addEventListener('blur',()=>set(false));row.addEventListener('click',()=>set(true))});
