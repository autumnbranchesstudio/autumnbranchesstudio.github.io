...
(()=>{
 ...
 words.forEach((w,i)=>{w.addEventListener('mouseenter',()=>{hoverIndex=i;pinStudioFocus(i);renderStudio(i)});w.addEventListener('mouseleave',()=>{hoverIndex=null;last=-1});w.addEventListener('focus',()=>{hoverIndex=i;pinStudioFocus(i);renderStudio(i)});w.addEventListener('blur',()=>{hoverIndex=null;last=-1})});
 function tickStudio(){if(studio&&hoverIndex===null){if(innerWidth>760){const r=studio.getBoundingClientRect(),max=Math.max(1,studio.offsetHeight-innerHeight),p=clamp(-r.top/max,0,1),centers=[0,.5,1];words.forEach((w,j)=>{const focus=clamp(1-Math.abs(p-centers[j])*2.25,0,1);w.style.opacity=(.3+.7*focus).toFixed(3);w.style.transform=`translateX(${(11*focus).toFixed(1)}px)`});const idx=p<.25?0:(p<.75?1:2);renderStudio(idx)}else{const sr=studio.getBoundingClientRect();if(sr.bottom>72&&sr.top<innerHeight){words.forEach(w=>{w.style.opacity='';w.style.transform=''});const targetY=innerHeight*.42;let idx=0,best=Infinity;words.forEach((w,j)=>{const wr=w.getBoundingClientRect(),d=Math.abs((wr.top+wr.bottom)/2-targetY);if(d<best){best=d;idx=j}});renderStudio(idx)}}}requestAnimationFrame(tickStudio)}if(words.length){renderStudio(0,true);requestAnimationFrame(tickStudio)}
 // Expertise strips: desktop reveals on hover/focus; mobile reveals progressively as each row enters the reading zone.
 ...
 serviceRows.forEach((row,i)=>{
   row.addEventListener('mouseenter',()=>{if(!mobileServices.matches)setService(i)});
   row.addEventListener('mouseleave',()=>{if(!mobileServices.matches)setService(-1)});
   row.addEventListener('focus',()=>{if(!mobileServices.matches)setService(i)});
   row.addEventListener('blur',()=>{if(!mobileServices.matches)setService(-1)});
   row.addEventListener('click',()=>setService(i));
 });
 let serviceRevealRaf=0;
 function updateMobileServices(){
   serviceRevealRaf=0;
   if(!mobileServices.matches||!serviceRows.length)return;
   const expertise=document.querySelector('.expertise');
   if(!expertise)return;
   const er=expertise.getBoundingClientRect();
   if(er.bottom<80||er.top>innerHeight){setService(-1);return}
   const triggerY=innerHeight*.66;
   let active=-1;
   serviceRows.forEach((row,i)=>{
     const rr=row.getBoundingClientRect();
     if(rr.top<=triggerY&&rr.bottom>72)active=i;
   });
   setService(active);
 }
 function queueMobileServices(){if(serviceRevealRaf)return;serviceRevealRaf=requestAnimationFrame(updateMobileServices)}
 addEventListener('scroll',queueMobileServices,{passive:true});
 addEventListener('resize',queueMobileServices,{passive:true});
 if(mobileServices.addEventListener)mobileServices.addEventListener('change',()=>{setService(-1);queueMobileServices()});
 else if(mobileServices.addListener)mobileServices.addListener(()=>{setService(-1);queueMobileServices()});
 queueMobileServices();
